  ...
  render() {
    ...
    return (
      <div
        ...
        className={`reactify-fullScreen__container ${this.state.expanded ? 'reactify-fullScreen--expanded' : ''} ${className || ''}`}
        {...otherProps}
      >
        {showIcon && <div className={`reactify-fullScreen__icon ${disabled ? 'reactify--disabled' : ''}`}>{iconToShow}</div>}
        {children}
      </div>
    );
  }
