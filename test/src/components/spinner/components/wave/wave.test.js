...
describe('Spinner Wave Component', () => {
    it('renders correctly', () => {
        const mountWrapper = mount(<Spinner wave/>);
        ...
        describe('should have correct theme when theme prop is passed', () => {
            ...
            Object.keys(ThemeColors).map(key => {
                it(`${key} theme`, () => {
                    const props = {};
                    props[key] = true;
                    const mountWrapper = mount(<Spinner wave {...props}></Spinner>);
                    const spinnerSpan = mountWrapper.findWhere(n => n.name() === 'span');
                    expect(spinnerSpan).toHaveStyleRule('border', expect.stringContaining(ThemeColors[key].color), { target: ':before'});
                });
            });
        });
    });
